...
lines = [line.split()[0] for line in open(
    sys.argv[4]).read().split('\n') if line]
...
cache = [[[0 for b in range(Z)] for i in range(Y)]
         for j in range(X)]  # cache[sets][blocks]
...
def main():
    ...
    for line in lines:
        ...
        if hit == 1:
            ...
            # adds the tag to the block in the LRU location
            cache[sets[i]][LRU][0] = tags[i]
            for z in range(Y):
                cache[sets[i]][z][1] += 1  # increments the LRU for each block
            # resets the LRU for the block being used
            cache[sets[i]][LRU][1] = 0
